...
export class DatabaseConnector {
	...
	/**
	 * Connect directly to database via Session Manager port forwarding
	 */
	async connectDatabase(environment: string, database: string = 'indicator'): Promise<void> {
		...
		const localPort = await this.findAvailablePort(defaultPort);

		if (localPort !== defaultPort) {
			console.log(chalk.yellow(`   Using port ${localPort} (${defaultPort} was in use)`));
			console.log('');
		}
		...
		console.log(chalk.green('🚀 Starting database tunnel...'));
		console.log(chalk.gray('   Once tunnel is established, run in another terminal:'));
		console.log(chalk.cyan(`   psql -h localhost -p ${localPort} -d ${dbInfo.DATABASE_NAME} -U ${dbInfo.DATABASE_USER}`));
		console.log('');
		console.log(chalk.gray('   Press Ctrl+C to close the tunnel'));
		console.log('');
